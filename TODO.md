...
| id | status | effort | description |
|----|--------|--------|-------------|
| SCAL-010 | open | S | Replace the `concurrency.py` worker contract that returns `list(eng)` with shard-to-file or chunk streaming so job size does not require accumulating a worker's complete output in memory. |
...
| id      | status | effort | description |
|---------|--------|--------|-------------|
| OBS-020 | open | S | Define a streaming proof-audit report schema and CLI destination/format, including redaction semantics, without dropping failures at a sample cap. |
