...
| CFG-022 | open | high | l | Compile composite child `transforms` as part of each nested field pipeline. `oneOf`, `weighted`, `sequence_of`, and distribution child specs currently accept malformed or unknown transform declarations but silently generate the untransformed source value; validate, prepare, apply, and prove nested transform chains consistently with top-level fields. |
| CFG-023 | open | high | m | Compile composite child `validators` as part of each nested field pipeline. Nested validator lists and references currently bypass structural/reference validation and never run, so a child such as an empty string with `validators: ["non_empty"]` validates and emits an invalid value. |
