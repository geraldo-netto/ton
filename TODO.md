...
|---|---|---|---|---|
| REL-051 | open | medium | small | Reliability/correctness: enforce zero-padding syntax in `ton/generators/integer.py::IntegerGenerator.prove`, rather than checking only parsed value and width. With bounds 1..99 and `padWithZero=true`, both ` 1` and `+1` pass proof although generation requires `01`. Add permanent malformed-padding regressions, including negative bounds, before fixing. |
| REL-052 | open | medium | small | Reliability/correctness: validate the fixed-point grammar in `ton/generators/decimal.py::DecimalGenerator.prove`. With equal bounds 1 and `decimals=3`, `1.0e0` and `1.0_0` pass because the implementation counts characters after the dot rather than decimal digits. Require the documented digits/scale/padding format; add permanent exponent, underscore, whitespace and valid-boundary regressions before fixing. |
| REL-053 | open | medium | small | Reliability/correctness: restrict phone proof placeholders to the generator's decimal-digit alphabet in `ton/generators/identity.py::PhoneGenerator.prove`. For `format="+#"`, `+²` passes because `str.isdigit()` accepts non-decimal numeral characters. Add permanent superscript/Unicode numeral rejection and ASCII-digit acceptance regressions before fixing. |
