...
| REL-049 | open | medium | small | Reliability/correctness: reject non-integral rational inputs in `ton/generators/base.py::coerce_int`. Equal integer bounds supplied as `Fraction(3, 2)` are truncated to 1, and generation emits `1` even with all proofs enabled, outside the caller's interval. Add permanent fractional/integral rational regressions for bounds, counts, and worker sequence settings; check exact integrality before conversion instead of restricting only floats and Decimals. |

### Configuration discoverability

| id | status | severity | effort | description |
|---|---|---|---|---|
| CFG-027 | open | medium | small | Configuration discoverability: use canonical reference resolution for transforms in `ton/_compiler.py::_resolve_transform` and `_transform_child_specs`. A field referencing `core.identity` fails with an explicit `transforms={"identity": IdentityTransform()}` registry, although core aliases resolve for generators. Add regression coverage for both spellings and distribution-child discovery with a bare-only transform registry. |
...
| CONC-022 | open | medium | small | Concurrency: require integer worker coordinates in `ton/concurrency.py::chunk_rows` and the worker entry points. `chunk_rows(10, 2.5, 0)` returns float 4.0, `worker_id=0.5` is accepted, and boolean coordinates pass; downstream failures then occur in seed packing or sequence offsets. Add permanent boolean/fractional/non-finite coordinate regressions and reject them before deriving seeds, offsets, or opening output. |
