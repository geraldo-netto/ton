...
| id      | status | effort | description |
|---------|--------|--------|-------------|
| REL-020 | open | M | Preserve fractional ISO bounds in `TimestampUnixGenerator`: `int(lo.timestamp())` and whole-second `duration_seconds()` currently turn a fixed bound such as `1970-01-01T00:00:00.900+00:00` into epoch `0`, which is outside the configured interval while `prove()` still accepts it. Derive inclusive representable bounds for seconds/millis (including pre-epoch values), reject only intervals with no representable value, and add boundary regressions. |
| REL-021 | open | S | Make `MACGenerator.prove()` parse separator-delimited octets structurally. The current `value.replace(separator, "")` also removes matching hexadecimal digits, so a permitted separator such as `"a"` makes generated output (for example `73aa9abeaf4af4adc`) fail its own proof. Cover hexadecimal-character separators, empty separators, case, OUI, and malformed layouts. |
...
| id      | status | effort | description |
|---------|--------|--------|-------------|
| PERF-013 | open | S | Precompute the lowercase given/family name membership sets used by `EmailGenerator.prove()`. Proof checking currently rebuilds both sets for every generated email, creating avoidable allocations on every row of large `all`/`audit` runs. |
