...
| id       | status | effort | description |
|----------|--------|--------|-------------|
| PLUG-016 | open | S | The claimed built-in class allowlist filters subclass discovery by `type_name`, not class identity (`_registry.py:45-48,342-363,387-413`). A subclass of `StringGenerator` that reuses `type_name = "string"` is discovered after its parent and replaces the core class in `_ensure_default_classes`, violating the non-shadowing guarantee. Build defaults directly from `BUILTIN_GENERATOR_CLASSES` (or filter by identity) and add a colliding-subclass test. |
| PLUG-017 | open | S | Programmatic `ExtensionCatalog.register_data_type` and `register_transform` accept arbitrary objects (`_registry.py:76-94`), while only validators receive a runtime protocol check. Reject invalid prototypes at registration so catalog construction fails locally instead of during deepcopy/validation/generation. |
| PLUG-018 | open | S | Transform entry-point validation checks only callable `prepare`, `apply`, and `prove` (`_registry.py:283-284`) but the compiler also requires `prepare_composite`, `type_name`, `capabilities`, and `config_keys`. Validate the complete public `Transform` protocol before registration so malformed plugins are skipped at the entry-point boundary. |
| PLUG-019 | open | S | Entry-point distribution metadata is stamped after the plugin is registered (`_registry.py:240-243,549-560`). If a valid slotted/read-only plugin rejects `_ton_plugin_*` attributes, the loader logs it as failed but leaves it mounted in the catalog. Store provenance in catalog-owned metadata or stamp before mutation so failed loads are transactional. |
